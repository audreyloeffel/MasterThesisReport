...
\begin{enumerate}
	...
	\item We have the sparse offsets given a set of indexes. But it does not take into account the possibly already non-null sparse offsets. This is the case when we take a view from a view. In this case we need to merge the two sparse offsets arrays.
...
\begin{algorithmic}
	\Statex
	\Function{CreateSparseOffsets}{int $offset$}\\
	\Comment{Compute the new offsets}
	...
	\\
	\\
	\Comment{Merge with sparseOffsets of this array}	
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\For{$oldDim=0$ to $hiddenDimensions.length$}
			\State $j\gets 0$
			\While{$newAxis[newDim] \leq hiddenDimensions[i]$}
