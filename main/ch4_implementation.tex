...
We also overallocated by a factor 2 the size of the databuffers used for storing the values and the indexes -> avoid to much reallocation mechanism
		...
		\For{$idxVir=0$ to $viewIndexes.length$}\\
		 ...
		 \Comment{The current dimension is hidden, it does not appear in the result}
			...
			\Else
				...
				\EndWhile\\
				\Comment{If the dimension is not fix, we add the offset to the coordinate}				
