...
\end{itemize}
 \item Sometimes the offsets are equal to zeros while having a non-null offset. In this case we need to override the offsets.
\begin{lstlisting}
...
// TODO % TODO
tensor contractions are not equal depending on the dimensions because the time to access the elements is not the same because the values are sorted along the first dimension
-> binary search, skip list, tree ... link to future chapter
...
Assuming we have a tensor with a shape equals to ($2\times 3\times 3$) as shown in figure \ref{fig:pointTensor}. We want to take a slice along the first dimension, assuming that the first dimension being the pages, the second the rows and the third the columns. We use the following operation:
\begin{lstlisting}
...
\end{algorithm}
	...
	\subsubsection{Example of the Sparse Offset Computation}
	// TODO LAYOUT
	%TODO Layout
	...
	\begin{lstlisting}
	myTensor.get(NDIndexArray.all(),
		NDIndexArray.point(0),
		NDIndexArray.point(3),
		NDIndexArray.all());
	\end{lstlisting}
	...
	\begin{figure}
		...
		flags = [1, 0, 0, 0, 0]
		\]
		\caption{Results from the resolution}
		\label{fig:resoSparseOffsets}
	\end{figure}
...
\begin{lstlisting}
	myTensor.get(NDIndexArray.all(),
		NDIndexArray.point(1),
		NDIndexArray.interval(0, 2),
